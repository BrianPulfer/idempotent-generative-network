...
def main(args):
    ...
    # Load datas
    normalize = Lambda(lambda x: (x-.5) * 2)
    ...
    callbacks = [ModelCheckpoint(monitor="val/loss", mode="min", dirpath="checkpoints", filename="best")]
    trainer = pl.Trainer(
        strategy='ddp',
        accelerator='auto',
    ...
    unnormalize = Lambda(lambda x: (x/2)+.5)
    transform = Compose([unnormalize, ToPILImage()])
    
    images = model.generate_n(10)
